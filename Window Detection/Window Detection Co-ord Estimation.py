...
#out = cv2.VideoWriter('output_coord_2.avi', fourcc, 20.0, (640, 480))
#inp = cv2.VideoWriter('input_box.avi', fourcc, 20.0, (640, 480))


#set length of the window
...
while True:
    ...
    # Create a mask to identify blue color regions in the HSV frame
                        ...
                        for i in range(4):
                            ...
                            cv2.putText(frame, str(int(corner_camera[0][0][0])) + "," + str(int(corner_camera[0][0][1])) + "," + str(
                                int(corner_camera[0][0][2])),(int(approx[i, 0, 0]), int(approx[i, 0, 1])),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.26, (0, 255, 0), 1)
